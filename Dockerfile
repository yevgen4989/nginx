...
FROM yevgen4989/alpine:${BASE_IMAGE_TAG}

ARG NGINX_VER

ARG SUPER_USER_ID=1000
ARG SUPER_GROUP_ID=1000
...
RUN set -ex; \
    ...
    \
	addgroup -g "${SUPER_GROUP_ID}" -S super; \
	adduser -u "${SUPER_USER_ID}" -D -S -s /bin/bash -G super super; \
	sed -i '/^super/s/!/*/' /etc/shadow; \
	echo "PS1='\w\$ '" >> /home/super/.bashrc; \
    \
    ...
    echo "find ${APP_ROOT} ${FILES_DIR} -maxdepth 0 -uid 0 -type d -exec chown super:super {} +" > /usr/local/bin/init_volumes; \
    chmod +x /usr/local/bin/init_volumes; \
    \
    { \
        echo -n 'super ALL=(root) NOPASSWD:SETENV: ' ; \
        echo -n '/usr/local/bin/init_volumes, ' ; \
        echo '/usr/sbin/nginx' ; \
    } | tee /etc/sudoers.d/super; \
    \
    chown super:super /usr/share/nginx/html/50x.html; \
    \
